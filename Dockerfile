...
ARG CLICKHOUSE_IMAGE=clickhouse/clickhouse-server:26.3-alpine
...
COPY --from=${CLICKHOUSE_IMAGE} /lib/libc.so.6 /lib/libdl.so.2 /lib/libm.so.6 /lib/libpthread.so.0 /lib/librt.so.1 /lib/libnss_dns.so.2 /lib/libnss_files.so.2 /lib/libresolv.so.2 /lib/ld-2.35.so /lib/
COPY --from=${CLICKHOUSE_IMAGE} /etc/nsswitch.conf /etc/
COPY --from=${CLICKHOUSE_IMAGE} /etc/clickhouse-server/config.d/docker_related_config.xml /etc/clickhouse-server/config.d/
COPY --from=${CLICKHOUSE_IMAGE} /entrypoint.sh /clickhouse-entrypoint.sh
...
COPY --from=${CLICKHOUSE_IMAGE} /usr/bin/clickhouse /usr/bin/clickhouse
RUN ln -s clickhouse /usr/bin/clickhouse-server && ln -s clickhouse /usr/bin/clickhouse-client
COPY --from=${CLICKHOUSE_IMAGE} /etc/clickhouse-server/config.xml /etc/clickhouse-server/users.xml /etc/clickhouse-server/
COPY --from=${CLICKHOUSE_IMAGE} /etc/clickhouse-client/config.xml /etc/clickhouse-client/
...
COPY --from=${POSTGRES_IMAGE} /usr/local /usr/local
...
COPY --from=${REDIS_IMAGE} /usr/local /usr/local
